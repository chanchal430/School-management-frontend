...
import { Search, Edit, Trash2, FileText, Calendar, Filter } from "lucide-react";
import DataTable from "../../ui/DataTable";
import FilterBar from "../../ui/FilterBar";
import FilterItem from "../../ui/FilterItem";
import { expenseList } from "../../../mock/expenseData";

const SearchExpense = () => {
  const [searchTerm, setSearchTerm] = useState("");
  const [dateFrom, setDateFrom] = useState("");
  const [dateTo, setDateTo] = useState("");
  const [filteredData, setFilteredData] = useState(expenseList);
  ...
  const actions = [
    {
      icon: FileText,
      label: "View Bill",
      onClick: (row) => alert(`Viewing bill: ${row.document || "No document attached"}`),
    },
    {
      icon: Edit,
      label: "Edit Expense",
  ...
  return (
    <div className="space-y-6">
      ...
      <FilterBar
        ...
        description="Filter institutional expenditures by date range or keyword."
      ...
      >
        <FilterItem label="Search Keyword">
          <div className="relative">
            ...
            <input
              type="text"
              placeholder="Expense title, invoice no..."
              className="w-full pl-10 pr-4 py-3 bg-slate-50 border border-slate-200 rounded-2xl focus:ring-2 focus:ring-rose-500/20 focus:border-rose-500 transition-all text-sm font-medium"
              value={searchTerm}
