...
import { phoneCallLogData } from "../../../mock/frontOfficeData";
import { PhoneIncoming, PhoneOutgoing, Phone, Plus, Search, Filter, RotateCcw, Edit2, Trash2 } from "lucide-react";
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <StatCard title="Total Calls Today" value="156" icon={<Phone size={20} />} color="indigo" />
        <StatCard title="Incoming Calls" value="98" icon={<PhoneIncoming size={20} />} color="emerald" />
        <StatCard title="Outgoing Calls" value="58" icon={<PhoneOutgoing size={20} />} color="rose" />
      </div>
      ...
      <SlideOver isOpen={isFormOpen} onClose={() => setIsFormOpen(false)} title="Add Phone Call Log">
        <form className="space-y-4" onSubmit={(e) => { e.preventDefault(); setIsFormOpen(false); }}>
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div className="space-y-1">
              <label className="text-[11px] font-black text-slate-500 uppercase tracking-widest">Next Follow Up</label>
              <input type="date" className="w-full px-4 py-2 bg-slate-50 border border-slate-200 rounded-lg text-sm font-bold text-slate-700 outline-none focus:ring-2 focus:ring-indigo-500/20" />
            </div>
            <div className="space-y-1">
              <label className="text-[11px] font-black text-slate-500 uppercase tracking-widest">Call Duration</label>
              <input type="text" placeholder="e.g. 05:20" className="w-full px-4 py-2 bg-slate-50 border border-slate-200 rounded-lg text-sm font-bold text-slate-700 outline-none focus:ring-2 focus:ring-indigo-500/20" />
            </div>
          </div>
          
          <div className="space-y-1">
            <label className="text-[11px] font-black text-slate-500 uppercase tracking-widest">Call Type</label>
            <div className="flex gap-4 mt-2 border border-slate-200 rounded-lg p-1 bg-slate-50">
              <label className="flex-1 text-center cursor-pointer relative">
                <input type="radio" name="callType" value="incoming" className="peer sr-only" defaultChecked />
              ...
              <label className="flex-1 text-center cursor-pointer relative">
                <input type="radio" name="callType" value="outgoing" className="peer sr-only" />
          ...
          <div className="pt-4 border-t border-slate-100 flex justify-end gap-3 mt-8">
            ...
            <button type="submit" className="px-6 py-2 bg-indigo-600 text-white rounded-xl font-bold hover:bg-indigo-700 transition-colors shadow-md shadow-indigo-200">Save Log</button>
          </div>
        </form>
      </SlideOver>
    </div>
