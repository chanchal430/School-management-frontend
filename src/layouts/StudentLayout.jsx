...
export default function StudentLayout() {
  ...
  return (
    <div className="min-h-screen bg-[#f8fafc] flex flex-col md:flex-row font-sans text-slate-900">
      ...
      {/* Sidebar */}
      <aside
        className={`fixed md:sticky top-0 inset-y-0 left-0 z-40 w-72 bg-[#022c22] text-slate-300 flex flex-col shadow-2xl h-screen transition-transform duration-300 ease-in-out no-scrollbar overflow-y-auto ${
        ...
        <header className="bg-white/70 backdrop-blur-xl sticky top-0 z-10 border-b border-slate-200/60 px-4 md:px-8 py-4 flex justify-between items-center h-20">
          ...
          <div className="flex items-center gap-3 md:gap-6">
            ...
            <div className="relative" ref={dropdownRef}>
              ...
              >
                <div className="w-8 h-8 md:w-10 md:h-10 rounded-xl bg-emerald-50 text-emerald-600 flex items-center justify-center font-bold text-base md:text-lg border border-emerald-100 shadow-inner group-hover:scale-95 transition-transform">
                  <User size={18} />
                </div>
                <ChevronDown
                  size={14}
                  className={`text-slate-400 mr-1 md:mr-2 transition-transform duration-300 ${isDropdownOpen ? "rotate-180 text-emerald-500" : ""
                    }`}
                />
              </button>
